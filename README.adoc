...
WARNING: This interface uses a  `java.util.Map` as the data structure root on Microstream, the `one.microstream.collections.lazy.LazyHashMap` provided by Microstream. It is crucial to the Id, annotated with a field with `jakarta.nosql.Id`,
...
WARNING: So, given the id: "any-id" it will belong to an entity, two entities with the same id will keep the last one updated.
...
WARNING: After mapping an entity, you can explore the advantage of using a `Template` interface, which can increase productivity on NoSQL operations.
